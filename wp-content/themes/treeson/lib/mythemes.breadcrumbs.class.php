...
class mythemes_breadcrumbs
{
	static function home()
    {
        ...
        $rett .= '<a href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( mythemes_mod( 'home-link-description' , __( 'go to home' , 'treeson' ) ) ) . '">';
        $rett .= '<i class="mythemes-icon-home"></i> <span>' . esc_html( mythemes_mod( 'home-label' , __( 'Home' , 'treeson' ) ) ) . '</span>';
        $rett .= '</a>';
        $rett .= '</li>';
    ...
    static function categories( $c_id )
	{
		...
		if( is_category( $c -> term_id ) ){
			...
			$rett .= '<li>';
			$rett .= '<a href="' . esc_url( $category_link ) . '" title="' . sprintf( __( 'See articles from category - %s' , 'treeson' ), esc_attr( $c -> name ) ) . '">' . esc_html( $c -> name ) . '</a>';
			$rett .= '</li>';
		}
	...
	static function pages( $p )
	{
        ...
        if( !is_page( $p -> ID  ) ){
            $rett .= '<li>';
            $rett .= '<a href="' . esc_url( get_permalink( $p -> ID ) ) . '" title="' . mythemes_post::title( $p -> ID, true ) . '">' . mythemes_post::title( $p -> ID, true )  . '</a>';
            $rett .= '</li>';
        }
    ...
    {
    	$label = _n( 'One Article' , '%s Articles' ,  absint( $query -> found_posts ) , 'treeson' );
